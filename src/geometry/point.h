...
template <typename T> struct Point {
  ...
  template <typename U> auto operator*(U o) const -> Point<decltype(x * o)> {
...
// Addition
template <typename T> Point<T> Point<T>::operator+(Point<T> o) const {
...
// Division
template <typename T> Point<T> Point<T>::operator/(const Point<T> o) const {
  return Point<T>{this->x / o.x, this->y / o.y};
}
template <typename T> Point<T> &Point<T>::operator/=(const Point<T> o) {
  this->x /= o.x;
...
// Constants
template <typename T> static const Point<T> ZERO = Point<T>(0, 0);
template <typename T> static const Point<T> ONE = Point<T>(T{1}, T{1});
template <typename T> static const Point<T> TWO = Point<T>(T{2}, T{2});
template <typename T> static const Point<T> TEN = Point<T>(T{10}, T{10});
